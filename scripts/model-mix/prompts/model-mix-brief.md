...
7. **LOCKED: probe → DeepSeek** — non-negotiable standing preference (user lock):
   - `global_defaults.probe` **must** be `{ "engine": "opencode", "model": "deepseek/deepseek-v4-pro" }` whenever `opencode` is in engines.
   ...
   - If `deepseek/deepseek-v4-pro` is missing from the model-pool excerpt, still use that slug and note the pool gap — do not silently switch models.
...
```STANDING_MIX_JSON
{
  ...
  "global_defaults": {
    ...
    "docs": { "engine": "...", "model": "..." },
    "probe": { "engine": "opencode", "model": "deepseek/deepseek-v4-pro" }
  },
  "locks": {
    "probe": { "engine": "opencode", "model": "deepseek/deepseek-v4-pro", "reason": "user lock" }
  }
}
```
